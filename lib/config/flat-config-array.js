...
class FlatConfigArray {
     ...
     * @param {string} [options.basePath] - Project root for resolving relative glob patterns
     */
    constructor(configs, { basePath = process.cwd() } = {}) {
    ...
    isFileIgnored(filePath) {
        ...
        for (const config of this.configs) {
            ...
            if (pathMatchesAnyPattern(relPath, config.ignores)) {
                return true;
            }
        }
        return false;
    ...
    getConfigForFile(filePath) {
        ...
        const relPath = path.relative(this.basePath, filePath);
        const posixPath = normalizePathForGlob(relPath);
